...
今回はHTTP/1.1のみ、Chunked Transferには触れずに解説します。

先に説明した通り、WebはHTTPのメッセージをやりとりすることで行われます。  
HTTP/1.1はテキストベースのプロトコルです。また、仕様として改行をCRLF(`\r\n`)で表します。  
...
* HTTPのプロトコル上の特徴を利用してWebサイトになにかしらの損害を与える方法を少なくとも1つ挙げよ。
  * 実装上の不備があると仮定しても良いがHTTP/1.1の仕様は満たしているものとする。
...
* 暗号化は自前でやると脆弱性を作りこむ危険性が高いのでなるべくTLSなど既存の仕様を利用しましょう。
* プロトコルの仕様は脆弱性の対処にも役立つ。
  * TLSの仕組みを知っておくことにより、脆弱性に対する対応が適切か判断できる。
